...
class MS_ResNet_34(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer3(x)
        x = self.LIF(x)
        x = self.avgpool(x) #self.avgpool = tdLayer(nn.AdaptiveAvgPool2d((1, 1)))
        x = torch.flatten(x, 2)
        x = self.fc1_s(x) #self.fc1_s = tdLayer(nn.Linear(512 * block.expansion, num_classes))
        return x
